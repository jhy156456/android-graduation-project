...
public class BestFoodRegisterInputFragment extends BaseFragment implements View.OnClickListener {
    ...
    private final int MAX_IMAGE_SELECTION_LIMIT = 20;
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        if (infoItem.post_category != 1002) { //등록액티비티가 구매 post에서온게 아닐경우
            sellPrice.setVisibility(View.GONE);
            osEdit.setHint("모집분야를 선택해주세요");
    ...
    public void finalUpload(int infoSeq) {
        for (int j = 0; j < (mSelectedImagesList.size() - 1); j++) {
            RemoteLib.getInstance().uploadFoodImagewithHandler(infoSeq,
                    saveImageItem[j].imageMemo, saveImageFile[j], finishHandler);
        }
    ...
    Handler finishHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (saveImageFileCheckCount == mSelectedImagesListCount) {
                ((MyApp) getActivity().getApplication()).setIsNewBestfood(true);
                ((MyApp) getActivity().getApplication()).setNewContest(true);
                progressOFF();
                context.finish();
            }
        }
